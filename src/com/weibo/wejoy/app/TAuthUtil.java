...
import com.weibo.wejoy.app.TAuthStore.TauthToken;
		...
		for (TauthToken tt : list) {
			...
			if (!today.equals(date)) {
				...
				try {
					...
					Map<String, String> params = new HashMap<String, String>(); // 增加泛型
																				// liuzhao
					params.put("app_secret", tt.secret);
					refreshTokenUrl = prop.getProperty("refreshTokenUrl",
							"http://i.api.weibo.com/auth/tauth_token.json?source="
									+ tt.source);
					ret = AppUtil.getInstance().requestPostUrl(refreshTokenUrl,
							null, params);

					// retry
					if (ret == null || ret.trim().isEmpty())
						ret = AppUtil.getInstance().requestPostUrl(
								refreshTokenUrl, null, params);

					JsonWrapper json = new JsonWrapper(ret.trim());
					String newToken = null;

					if ((newToken = json.get("tauth_token")) != null
							&& !newToken.trim().isEmpty()) {
						newToken = newToken.trim();
						tt.token = newToken;
						log.info("refreshTauthTokenFromApi end! token change! source:"
								+ tt.source + ", new token:" + newToken);
					} else {
						String info = "refreshTauthTokenFromApi error! ret:"
								+ ret + ", url:" + refreshTokenUrl;
						log.error(info);
					}
				} catch (Exception e) {
					String info = "refreshTauthTokenFromApi error! ret:" + ret
							+ ", url:" + refreshTokenUrl;
					log.error(info, e);
				}
			}
